...
module.exports=async(req,res)=>{
  ...
  if(action==='send'){
    ...
    try{
      ...
      if(!String(participant.email||'').trim())return res.status(400).json({error:'EMAIL_MISSING'});
      if(participant.confirmationEmailSent===true)return res.status(200).json({sent:false,alreadySent:true});
      return res.status(200).json(await sendRegistrationConfirmation(ref));
    }catch(e){
      console.error('confirmation send failed',e);
      return res.status(500).json({error:'CONFIRMATION_SEND_FAILED',message:String(e.message||e).slice(0,200)});
    }
  }
  ...
  if(action==='recover'){
    try{
      ...
      for(const doc of candidates){
        ...
        catch(e){errors++;console.error('confirmation recovery item failed',doc.id,e);}
      }
      return res.status(200).json({attempted:candidates.length,sent,errors,remaining:Math.max(0,pending.length-sent)});
    }catch(e){
      console.error('confirmation recovery failed',e);
      return res.status(500).json({error:'CONFIRMATION_RECOVERY_FAILED',message:String(e.message||e).slice(0,200)});
    }
  }
  ...
  try{
    ...
    console.error('confirmation test failed',e);
    return res.status(500).json({error:'CONFIRMATION_TEST_FAILED',message:String(e.message||e).slice(0,200)});
  }
};
